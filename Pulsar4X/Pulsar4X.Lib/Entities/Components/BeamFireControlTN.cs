    ...
    public class BeamFireControlTN : ComponentTN
    {
        ...
        public bool FireWeapons(float DistanceToTarget, Random RNG, int track, ShipTN FiringShip)
        {
            if (DistanceToTarget > m_oBeamFireControlDef.range || (m_lLinkedWeapons.Count == 0 && m_lLinkedTurrets.Count == 0) || isDestroyed == true)
            {
                ...
                {
                    MessageEntry NMsg = new MessageEntry(MessageEntry.MessageType.FiringZeroHitChance, FiringShip.ShipsTaskGroup.Position.System, FiringShip.ShipsTaskGroup,
                    ...
                    foreach (BeamTN LinkedWeapon in m_lLinkedWeapons)
                    {
                        if (LinkedWeapon.beamDef.range > DistanceToTarget && LinkedWeapon.readyToFire() == true)
                        {
                            ...
                            if (weaponFired == true)
                            {
                                for (int BeamShotIterator = 0; BeamShotIterator < LinkedWeapon.beamDef.shotCount; BeamShotIterator++)
                                {
                                    ...
                                    if (toHit >= Hit)
                                    {
                                        ...
                                        MessageEntry NMsg = new MessageEntry(MessageEntry.MessageType.FiringHit, FiringShip.ShipsTaskGroup.Position.System, FiringShip.ShipsTaskGroup,
                                    ...
                                    else
                                    {
                                        ...
                                        MessageEntry NMsg = new MessageEntry(MessageEntry.MessageType.FiringMissed, FiringShip.ShipsTaskGroup.Position.System, FiringShip.ShipsTaskGroup,
                                                                             GameState.Instance.GameDateTime, GameState.Instance.LastTimestep, WeaponFireS);
                            ...
                            else if (LinkedWeapon.isDestroyed == false)
                            {
                                ...
                                MessageEntry NMsg = new MessageEntry(MessageEntry.MessageType.FiringRecharging, FiringShip.ShipsTaskGroup.Position.System, FiringShip.ShipsTaskGroup,
                                                                     GameState.Instance.GameDateTime, GameState.Instance.LastTimestep, WeaponFireS);
